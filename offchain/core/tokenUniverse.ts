...
// Format: [TokenA, TokenB] where TokenA and TokenB are symbol keys from TOKENS
export const PAIRS = [
...
export function getTokenAddress(chainId: number, symbol: string): string | null {
  ...
  return (chainTokens as any)[symbol] || null;
}
 ...
 * @param chainId - EIP-155 Chain ID
 * @returns Array of [tokenA_address, tokenB_address] pairs
 */
export function getAvailablePairs(chainId: number): Array<[string, string]> {
  const chainTokens = TOKENS[chainId as keyof typeof TOKENS];
  if (!chainTokens) return [];
  
  const availablePairs: Array<[string, string]> = [];
 ...
 */
export function getTokenSymbol(chainId: number, address: string): string | null {
  const chainTokens = TOKENS[chainId as keyof typeof TOKENS];
  if (!chainTokens) return null;
 ...
 */
export function isPairSupported(chainId: number, tokenA: string, tokenB: string): boolean {
 ...
 * @param chainId - EIP-155 Chain ID
 * @returns Object mapping symbol to address
 */
export function getChainTokens(chainId: number): Record<string, string> {
  const chainTokens = TOKENS[chainId as keyof typeof TOKENS];
  if (!chainTokens) return {};
